...
public class Ae5 {
    ...
    public static int nodeDepths(BinaryTree root) {
        return accumulate(root, 0).stream().mapToInt(Integer::intValue).sum();
    }

    private static List<Integer> accumulate(BinaryTree tree, int sum) {
        if (tree.right == null && tree.left == null) return Collections.singletonList(sum);
        else {
            List<Integer> results = Arrays.asList(sum);
            if (tree.left != null) results.addAll(accumulate(tree.left, 1 + sum));
            if (tree.right != null) results.addAll(accumulate(tree.right, 1 + sum));
            return results;
        }
    }
